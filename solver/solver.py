...
class FEMSolver2D:
    ...
    def assemble(self):
        GlobalStiffness = np.zeros((self.mesh.points.shape[0], self.mesh.points.shape[0]))
    ...
    def plot2d(self):
        ...
        plt.tricontourf(self.mesh.points[:, 0], self.mesh.points[:, 1], self.mesh.elements,
                        self.solution, levels=14, cmap="viridis")
        plt.triplot(self.mesh.points[:, 0], self.mesh.points[:, 1], self.mesh.elements, color='black', alpha=0.3, linewidth=0.5)
    ...
    def plot3d(self):
        ...
        ax.plot_trisurf(self.mesh.points[:, 0], self.mesh.points[:, 1], self.solution,
                        triangles=self.mesh.elements, cmap='viridis', edgecolor='none')
